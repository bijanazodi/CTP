...
def analyze_crypto(crypto_id, crypto_symbol, latest_price, days=10, interval='hourly'):
    try:
        ...
        # Use the valid trading pair for Binance
        binance_symbol = crypto_symbol.upper() + 'USDT'

        data = fetch_binance_data(binance_symbol, interval, 24 * days)
        if data is None or data.empty:
            logging.warning(f"Failed to fetch data from Binance for {crypto_symbol}. Trying CoinGecko...")
